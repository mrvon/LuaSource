...
function clone(object)
    ...
    local function __copy(object, deep)
        if type(object) ~= "table" then
            debug_print("Case 1: " .. tostring(object), deep)
            return object
        elseif lookup_table[object] then
            debug_print("Case 2: " .. tostring(lookup_table[object]), deep)
            return lookup_table[object]
        end
        debug_print("-------- Case 3 start --------", deep)
        local new_table = {}
        lookup_table[object] = new_table
        for key, value in pairs(object) do
            new_table[__copy(key, deep + 1)] = __copy(value, deep + 1)
        end
        debug_print("-------- Case 3 end ----------", deep)
